...
class ThreadTestCase(TestCase):
    ...
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Estimado")
        message2 = Message.objects.create(user=self.user2, content="Estimada")
    ...
    def test_find_thread_with_custom_manager(self):
        self.thread.users.add(self.user1, self.user2)
        # Se creo el metodo objects con custom manager para evitar los filters
